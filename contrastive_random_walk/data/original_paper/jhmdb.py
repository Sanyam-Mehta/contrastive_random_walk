...
class JhmdbSet(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        lbls_coord[0, :, :] = (
            lbls_coord[0, :, :] * float(neww) / float(wd) / self.mapScale[0]
        )
        lbls_coord[1, :, :] = (
            lbls_coord[1, :, :] * float(newh) / float(ht) / self.mapScale[1]
        )
        lblsize = (rsz_h, rsz_w)

        lbls = np.zeros(
            (lbls_coord.shape[2], lblsize[0], lblsize[1], lbls_coord.shape[1])
        )
        ...
        lbls_tensor = torch.zeros(
            frame_num, lblsize[0], lblsize[1], lbls_coord.shape[1]
        )
        ...
        lbls_tensor = torch.cat(
            [(lbls_tensor.sum(-1) == 0)[..., None] * 1.0, lbls_tensor], dim=-1
        )
