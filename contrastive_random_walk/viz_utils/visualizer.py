...
class Visualizer():
    def __init__(self, config):
        ...
        if self.tf_log:
            ...
            self.tf = tf
            self.log_dir = os.path.join(config.OUTPUT_DIR, config.VISUALIZE.NAME, 'logs')
            ...
            for label, image_numpy in visuals.items():
                if isinstance(image_numpy, list):
                    for i in range(len(image_numpy)):
                        img_path = os.path.join(self.img_dir, 'step%.3d_%s_%d.jpg' % (step, label, i))
                        util.save_image(image_numpy[i], img_path)
                else:
                    img_path = os.path.join(self.img_dir, 'step%.3d_%s.jpg' % (step, label))
