...
def tensor2im(image_tensor, imtype=np.uint8, inputmode=""):
    ...
    if inputmode == "div255-mean":
        ...
        image_numpy = np.transpose(
            (image_numpy - i_min) / (i_max - i_min) * 255.0, (1, 2, 0)
        )
    elif inputmode == "bgr-mean":
        image_numpy = np.transpose(image_numpy, (1, 2, 0))[:, :, ::-1] + np.asarray(
            [122.675, 116.669, 104.008]
        )
        # print(image_numpy.max(),image_numpy.min())
    elif inputmode.lower() == "pil":
        img = np.array(transforms.ToPILImage()(image_tensor))
        return img
    else:
        # print('depth')
