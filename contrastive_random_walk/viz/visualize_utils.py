...
def draw_matches(image_1, image_2, embeddings_image_1, embeddings_image_2):
    ...
    matches = bf.match(embeddings_image_1.cpu().detach().numpy(), embeddings_image_2.cpu().detach().numpy())

    scale = image_1.shape[-2] / height

    grid = torch.stack([torch.arange(0, height)[None].repeat(height, 1), torch.arange(0, height)[:, None].repeat(1, height)])
    ...
    out = cv2.drawMatches(image_1.astype(np.uint8), kps, image_2.astype(np.uint8), kps,matches[:], None, flags=2).transpose(2,0,1)
...
def pca_feats_top_3K_components(embeddings_all_images, K=1):
    ...
    pca_output = torch.tensor(pca.fit_transform(embeddings_all_images.reshape(-1, embeddings_all_images.shape[-1]).numpy()))

    pca_output_reshaped = pca_output.reshape(embeddings_all_images.shape[0], embeddings_all_images.shape[1], embeddings_all_images.shape[2], 3*K)
    ...
    pca_output_all_images = [
        (pca_output_all_images[i] - pca_output_all_images[i].min()) / (pca_output_all_images[i].max() - pca_output_all_images[i].min()) for i in range(len(pca_output_all_images))
    ]
